...
Networks in \ak\ are represented as graphs, thus the fixed point path search is a graph search problem. A network graph is a directed multigraph because in \ak\ two nodes are not restricted to be connected with more than one edge. The graph has four types of nodes, namely a box, a synchroniser, a merger and a network. The search problem can be formulated as follows. In a directed multigraph with typed nodes find a path that traverses only nodes of a specified type and prove that the path is unique. A recursive depth-first search based path enumeration algorithm in Fig. \ref{fig:dfs} solves the problem in TODO complexity.
...
%\subsubsection{Extracting the fixed point condition from a synchroniser}
...
\end{lstlisting}
\caption{The start state of a synchroniser that encodes the fixed point condition $p$ on channel $x$}
\label{ffp:synch_fp}
\end{figure}

We shall note, that there exists no more transition on the channel from the start state with the same structure (a single \emph{send} clause). Otherwise, the synchroniser has the fixed point condition that is a disjunction of conditions in such transitions. The algorithm in Fig. \ref{fig:ffp_extract} checks if a forward fixed point exists in channel $x$ and extracts the fixed point condition from the synchroniser source code. The algorithm supports the renaming of channel $x$ and makes sure that the fixed point path does not branch in the synchroniser. It is assumed that channel $x$ is declared as an input and an output channel of the synchroniser.

\begin{figure}[h!]
\noindent\fbox{%
\begin{minipage}{\dimexpr\linewidth-2\fboxsep-2\fboxrule\relax}
\begin{algorithmic}[1]
% Input: code - the AST of the synchroniser program, x - the input label of the fixed point channel
% Output: CondDict (a, CondList), where a is the output label of the fixed point channel, CondList is the list of atomic fixed point conditions
...
\begin{minipage}{\dimexpr\linewidth-2\fboxsep-2\fboxrule\relax}
\begin{algorithmic}[1]
% Input: an operand network, a channel to test for a forward fixed point
% Output: FFPCondList - a list of lists which are the fixed point conditions extracted from the synchronisers that traverse the fixed point path
\Function{detect\_ffp}{$net, x$}
  ...
  \State $graph\gets get\_graph$($net$)\Comment{\emph{extract the graph from the network data structure}}
% maybe remove get_graph and leave just net?
  \State $FFPCondList\gets get\_cond\_list$($graph, x, start, end$)
\EndFunction


\Function{get\_cond\_list}{$graph, x, start, end, cond\_list=[]$}
  \If{$start$ \emph{is a box}}
    \State \textbf{return} TODO
  \EndIf

  \If{$start$ \emph{is a synchroniser}}
    \State ($out\_port,fp\_list$)$\gets extract\_ffp$($start, x$)
    \State $cond\_list\gets cond\_list$ \textbf{..} $fp\_list$
    \For{\textbf{each} \emph{succ\_node in succ\_nodes}($start$)}
      \If{$get\_label$($edge$($start, succ\_node$))$ = out\_port$}
        \State ...$\gets get\_cond\_list$($graph, out\_port, succ\_node, end, cond\_list$)
      \EndIf
    \EndFor
  ...
  \If{$start$ \emph{is a network}}
    \State \textbf{return} $detect\_ffp$($net,x$)
  \EndIf
...
\caption{A forward fixed point detection in channel $x$\label{fig:ffp_detect}}
\end{figure}
