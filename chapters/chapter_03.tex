...
\item If the search encounters a synchroniser, the fixed point condition of the synchroniser is extracted using the function $extract_ffp$ in Fig. \ref{fig:ffp_extract}. If the synchroniser has no fixed point condition, the traversed path is rejected. Otherwise, the search continues only for the successors of the node that were detected by $extract_ffp$
...
\begin{minipage}{\dimexpr\linewidth-2\fboxsep-2\fboxrule\relax}
\begin{algorithmic}[1]
\Require an operand network graph ($graph$), a channel to test for a forward foxed point ($x$)
%\Ensure
\Statex

\Function{get\_cond\_list}{$graph, x, start, end, cond\_list=empty\_list$}
  ...
  \If{$start$ \emph{is a synchroniser}}
    ...
    \For{\textbf{each} \emph{succ\_node in succ\_nodes}($start$)}
      ...
      \If{$out\_port \in keys$($CondDict$)}
        ...
        \State $NewLists\gets get\_cond\_list$($graph, out\_port, succ\_node, end, cond\_list$)

        \For{\emph{new\_list in NewLists}}
          \State $Lists\gets Lists$ \textbf{..} $new\_list$
        \EndFor
%If Lists contains more than one list - the fixed point path branches
  ...
  \If{$start$ \emph{is a merger}}
    ...
    \State \textbf{return} $get\_cond\_list$($graph, out\_port, succ\_node, end, cond\_list$)
  \EndIf

  \If{$start$ \emph{is a network}}
    \State \textbf{return} $detect\_ffp$($start,x$) %it can be empty_list as well if the network does not have a fixed point, then the path is rejected
  \EndIf
...
\caption{TODO\label{fig:ffp_get_cond}}
\end{figure}
