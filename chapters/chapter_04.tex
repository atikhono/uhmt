...
%Now the \ak\ compiler can check the fix-point out of the network topology because there's a channel connected to the box that.
    ...
    \subsection{Another Approach to the Forward Fixed Point\label{approach}}
...
A net in Fig. \ref{fig:ffp_new} has two input and output channels $a$ and $x$. Channel $x$ is the fixed-point channel. Output to the channel $x$ is produced by box $B_3$. Its output channel is wired with $x$ via merger $M$.
