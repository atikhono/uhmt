...
\item Algorithm of the execution of synchroniser
...
The compiler we have implemented does not optimise the code deeply. We have only implemented an elimination of unused states. However, based on the synchroniser execution algorithm we provided, a wider set of dead code elimination optimisations can be implemented\footnote{E.g. all transitions that are specified after the \texttt{.else} transition on a particular channel can be removed}.
