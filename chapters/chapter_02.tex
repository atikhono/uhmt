...
In this chapter we describe the source language for an \ak\ synchroniser and implement a compiler for the language. Prior to describing the language in details, we present a mathematical model of a synchroniser from \cite{astrakahn} and describe sources of non-determinism in synchronisers providing the execution algorithm of a synchroniser.

The $aksync$ compiler is integrated into the current \ak\ runtime system prototype. It takes the source code of a synchroniser program and generates the runtime object. The compiler performs the syntactic and type checking of a program and generates the CAL passport of the synchroniser.
...
(\Phi, \; \Pi),\\\nonumber
\end{equation}
where $\Phi = (A, \; S, \; T)$ is a nondeterministic state machine with the alphabet of events $A \subseteq C \times P$, $C$ denotes the set of the synchroniser input channels and $P$ denotes the set of the predicates on channel messages\footnote{An event $(c, \; p) \in A$ represents the reception of a message on channel $c$ that satisfies the predicate $p$}. The set of abstract states in $\Phi$ is denoted as $S \supseteq \{s_{0}\}$ with the start state $s_{0}$, $T \: : \: A \times S \to S$ is the transition matrix of $\Phi$. $\Pi \: : \: S \times \Omega \to V$ is the path functional that defines the synchroniser output with the set of output channels $\Omega$ and the set of message values.
...
\end{equation}
where $s_{0}$ is the start state and $c_{i} \in C$, $0 \le i \le k$ is the channel that caused the transition from the state $s_{i}$.

Let $\mu_{i}$ be the message received in the transition from the state $s_{i}$. Then
\begin{equation}
\Pi \; (s_{k}, \omega_{m}) = \psi_{\sqcap} \; \{\mu_{i} \: | \: \rho_{ki}^{m} \; (s_{i}), \: 0 \le i \le k\},\\\nonumber
\end{equation}
...
The state machine can have a regular structure whereby many transitions can be defined at once by a formula with some limited range integer variables. For example, a machine with 8 states could have a transition matrix defined thus: $S_{k \; mod \; 8} \to S_{k+1 \; mod \; 8}$. In order to be able to use regular structures, \ak\ allows synchronisers to declare \emph{state} variables.
...
\textbf{Example: the binary zip synchroniser}  Zip2 receives messages on its input channels and sends their concatenation to the output channel. In the resulting concatenation there is exactly one message from each input channel and those messages are ordered as they received.
...
So far we have given a formal definition of a synchroniser. Along with it we have introduced two important concepts for a synchroniser that are considered as a part of the synchroniser language. First is a message storage mechanism we call a store variable. Second is a mechanism to define big transition matrices with a regular structure we call a state variable.
...
%For example for Choice: $union \; (: \: a:t_a \: :) \: (: \: a:t_b \: :) = (: \: a:union \: t_a, \: t_b \: :)$. Same for record.
...
In order to achieve the highest throughput, an \ak\ synchroniser exploits a non-deterministic behaviour. In a certain state more than one input channel may be ready, however, a state machine receives one input message at a time. The synchroniser does not take any transition that potentially causes sending to a blocked channel. Among the transitions that do not send to blocked channels which will be triggered is defined by the fairness policy: when more than one transition is possible in a given state, all choices will be made with the same frequency. If transitions are prioritised, the choices are made within each scope at first.

Once the transition has been chosen and the associated actions have been executed, the synchroniser transits to the destination state. In order to avoid the transition to a state, in which there are no ready input channels, the synchroniser may be provided with a set of destinations to choose. In the synchroniser language such behaviour is expressed in a goto clause with multiple states. The algorithm in Fig. \ref{fig:execod_alg} defines the ordering of the choices that a synchroniser makes during the execution.
...
TODO
The compiler generates a runtime object that is passed to the \ak\ runtime system for the execution.
...
The compiler we have implemented does not optimise the code deeply. We have only implemented an elimination of unused states. However, based on the synchroniser execution algorithm we provided, a wider set of dead code elimination optimisations can be implemented\footnote{E.g. all transitions that are specified after the \texttt{.else} transition on a particular channel can be removed}. TODO this is not right.
