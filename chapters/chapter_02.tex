...
The path functional $\Pi \: (s_k, \; \omega_m) \in \Pi$\replaced{, where $\Omega$ is the set of output channels,}{$ \: : \: S \times \Omega \to V^{(*)}$} defines the output to channel $\omega_m \in \Omega$ of the synchroniser \replaced{in state $s_k \in S$}{and $V$ is the set of message values, $V^{(*)}$ denotes a set of all sequences from $V$}. The functional $\Pi \: (s_k, \; \omega_m)$ is based on the retrospective sequence of transitions from the most recent visit to the start state $s_{0}$ to $s_{k}$:
\begin{equation}
