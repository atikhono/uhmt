...
The $aksync$ compiler is integrated into the current \ak\ runtime system prototype. It takes the source code of a synchroniser program, performs the syntactic and semantic analysis of it, builds a data structure for the \ak\ runtime and generates the \replaced{communication}{CAL} passport of the synchroniser.


    \section{Mathematical Model}
\added{Synchronisers are finite state machines for joining received messages and sending them on to the output channels. Synchronisers can have one or more input and output channels. }From the mathematical point of view a synchroniser is a pair
\begin{equation}
(\Phi, \; \Pi),\\\nonumber
\end{equation}
where $\Phi = (A, \; S, \; T)$ is a nondeterministic state machine with the alphabet of events $A \subseteq C \times P$, where $C$ denotes the set of input channels and $P$ the set of the predicates on channel messages. An event $(c, \; p) \in A$ represents the reception of message\added{ $\mu$} on channel $c$ that satisfies the predicate $p$. The set of abstract states in $\Phi$ is denoted as $S\;$\replaced{$ = \{s_0, \dots, s_k, \dots, s_n\}$, $k \leq n$,}{$\supseteq \{s_{0}\}$} with the start state $s_{0}$, and the transition matrix as $T \: : \: A \times S \to S$.

The path functional $\Pi \: (S, \; \Omega)$\replaced{, where $\Omega$ is the set of output channels,}{$ \: : \: S \times \Omega \to V^{(*)}$} defines the output of the synchroniser\deleted{, where $V$ is the set of message values, $V^{(*)}$ denotes a set of all sequences from $V$}.\added{ The function $\Pi \: (s_k, \; \omega_m) \in \Pi$ defines the output to channel $\omega_m \in \Omega$ of the synchroniser in state $s_k \in S$.} In state $s_k$ the functional is based on the retrospective sequence of transitions from the most recent visit to the start state $s_{0}$ to $s_{k}$:
\begin{equation}
...
\end{equation}
where $\rho_{ki}^{m}$ is the selection predicate that defines $\Pi$, and $\psi_{\sqcap}$ is the operator that coerces the messages in the operand set to their joint greatest subtype.\added{ A synchroniser in Fig. \ref{fig:math_exec} emits to channel $\omega_m$ only a message received on channel $c_k$ in state $s_k$. Fig. \ref{fig:math_exec} shows the path functionals for $\omega_m$ that correspond to each transition on channel $c_i$ from state $s_i$, $0 \leq i \leq k$.}
...
\textbf{Example: the counter synchroniser}  Counter sends every $n$-th message from its input channel to the output channel, other messages are disregarded. The transition diagram of the counter synchroniser\added{ with input channel $a$ and output channel $c$} for $n = 3$ is given in Figure \ref{fig:chapter_02_cnt_unroll}.
...
%\deleted{The counter synchroniser is a pair $(\Phi, \; \Pi)$, where
%  \begin{itemize}
%  \item[] $\Phi = (A, \; S, \; T)$,
%    \begin{itemize}
...
%  \end{itemize}
%
%An output message is emitted when a transition from the state $s_{2}$ happens. This state is reached in a single path:
%  \begin{itemize}
%  \item[]
...
In a given state $s_k$ for each output channel $\omega_{m}$ we note all $\rho_{ki}^{m}$ that are true. The corresponding message values must be saved in previous states and recalled in state $s_k$. It is expected that the boolean vector $[\rho_{ki}^{m}]$ has only very few true elements. Although the functional $\Pi$ can be implemented as function that retrieves all the saved messages at once, it is feasible to implement it in the form of individual \emph{store} variables. \replaced{A store variable is associated with an input channel}{The type of a store variable is determined when a variable is assigned}.

\textbf{Example: the binary zip synchroniser}  Zip2 receives messages on its input channels and sends their concatenation to the output channel. In the resulting concatenation there is exactly one message from each input channel and those messages are combined for the output. The transition diagram of the zip2 synchroniser with input channels $a$ and $b$ and output channel $c$ is given in Fig. \ref{fig:zip2}. Store variables $ma$ and $mb$ are associated with the input channels $a$ and $b$ respectively.


  \begin{figure}[here]
  \centering
  \includegraphics[scale=0.8]{figs/chapter_02_zip2.pdf}
...
So far we have given a formal definition of the synchroniser. Along with it we have introduced two important concepts for the synchroniser that are considered as a part of the synchroniser language. First is a message storage mechanism we call a store variable. Second is a mechanism for defining large regular transition matrices, which we call a state variable.\added{ In the next section we describe the programming language of synchronisers.} We shall note that the language allows some operations on store variables\footnote{E.g. merging two store variables into another one, appending a message extension that was computer by the synchroniser into a store variable, etc.} that the presented mathematical model cannot describe.\added{ The model should be fixed in accordance with the synchroniser language in the future.}
...
The state variable $foo$ that is declared on the line 2 in Fig. \ref{sync_statevar} can only be assigned the values $d$, $e$ and $f$ specified in the enumeration. The enumeration values are integer constants. If they are not specified explicitly, they are assigned consecutive positive integers starting with 0.
...
Store expression is a mechanism to combine data. In \ak\, data are typed. Types are \replaced{MDL}{CAL} terms. The result of the store expression can be either saved in a store variable or sent down an output channel.
...
%  \Statex
  ...
  \Statex
  \State $channel\gets$ \emph{the least frequently taken channel from \textbf{ReadyInputs}}
  \State $message\gets$ \emph{fetch a message from \textbf{channel}}

%  \Statex
  \Comment{\emph{iterate over transitions sorted by the number of executions from low to high}}
  \For{\textbf{each} number of transition executions \emph{N in} $curr\_state$}
  %\For{\textbf{each} \emph{priority} $P$ \emph{in} $curr\_state$}
  %  \Comment{\emph{iterate over transition priorities from the highest to the lowest}}
    \State $trans\gets$ \emph{all transitions with the number of executions N that read from \textbf{channel}}
    \State $valid\_trans\gets$ \emph{all transitions from \textbf{trans} with satisfied conditions}
    \State $else\_trans\gets$ \emph{the} \texttt{.else} \emph{transition from \textbf{trans} with the satisfied condition}
...
The lexical and syntax analysers are implemented using PLY \cite{ply} - an implementation of lex and yacc for Python. The semantic analyser performs semantic and type checking. The code generator emits the synchroniser's runtime data structure and derives its\deleted{ CAL} passport.
...
The attribute \emph{type} is added to each entry in the symbol table. State variables are assigned type \emph{integer}. Non-integer variables, whose structure is unknown, are assigned a special type \emph{void}, which stands for a variable \replaced{MDL}{CAL} term. A detailed information about the synchroniser language type \replaced{checker}{system} is provided in section \ref{type_check}.
...
\Function{get\_states}{$synch$}
  \State $StateSet\gets \added{nil}$
  \State $GotoSet\gets \added{nil}$
  \For{\textbf{each} \emph{state in state\_list(sync)}}
    \If{$label(state) \in StateSet$}
      \State \emph{error}\Comment{\emph{if the state label is not unique}}
    \Else
      \State $\added{StateSet}\gets \added{StateSet}$.Append($label(state)$)
    \EndIf
    \Statex
    \For{\textbf{each} \emph{trans in trans\_list(state)}}
      \For{\textbf{each} \emph{gotostate in goto\_list(trans)}}
        \If{$gotostate \notin GotoSet$}
          \State $GotoSet\gets GotoSet$.Append($gotostate$)
        \EndIf
      \EndFor
    \EndFor
  \EndFor
...
The communication protocol of the \ak\ runtime system prototype supports only choices. When a synchroniser reads a message from its input channel, the record that belongs to the variant specified in the synchroniser transition is instantiated\footnote{this means that store variables cannot keep multiple variants}. We take this into account in implementing the type \replaced{checker}{system} of the synchroniser language. Because the synchronisers can read values only of the fields that are known to be integer, the basic types in the synchroniser language are integer and \replaced{MDL}{CAL} variable. Integer is the type of state variable. \replaced{MDL}{CAL} variables are building blocks for the only constructed type in the synchroniser language -- a record. A record is constructed by the concatenation of two records. The pseudo-code of the record constructor $||$ is given in Fig. \ref{rec_construc}. The record constructor is obviously commutative and associative.
...
The case when a label-value pair labeled $l$ exists in both operand records $r_1$ and $r_2$ is indicated with $union(r_1(l),r_2(l))$. The \replaced{constraint}{CAL} solver resolves which option to take during the constraint aggregation pass in the \ak\ compiler.
%When labels intersect, it is the deal for CAL solver to resolve with option to take (it should consider flow inheritance).

We describe the type \replaced{checkers}{systems} in terms of grammar productions and corresponding semantic actions. The type \replaced{checkers}{systems} for state and store expressions are given in Fig. \ref{ts_int_exp} and Fig. \ref{ts_data_exp} respectively. The synthesized attribute \emph{type} for an expression \iangled{E} gives the type of the expression assigned by the type system for the expression generated by \iangled{E}. The type \replaced{checker}{system} for statements is given in Fig. \ref{ts_stmt}. It assures that the left hand side can be assigned to.
...
\end{tabular*}
\caption{Type \replaced{checker}{system} for statements\label{ts_stmt}}
%\end{sidewaystable}
\end{figure}
...
\end{tabular*}
\caption{Type \replaced{checker}{system} for state expressions\label{ts_int_exp}}
%\end{sidewaystable}
\end{figure}
...
\end{tabular*}
\caption{Type \replaced{checker}{system} for store expressions\label{ts_data_exp}}
%\end{sidewaystable}
\end{figure}
...
The compiler generates \replaced{an abstract syntax tree (AST)}{a data structure} that is passed to the \ak\ runtime system for the interpretation. \added{An example of the AST for the counter synchroniser in Fig. \ref{sync_ast} is shown in Fig. \ref{sync_ast}. A detailed description of the AST structure can be found in Appendix \ref{ast_app}.}
...
\Function{input\_term}{$synch$}
  ...
  \Statex
  \For{\textbf{each} \emph{state in state\_list(sync)}}\Comment{\emph{Construct a dictionary $CondDict$ with input ports as keys and sets of corresponding message conditions as values}}
    \For{\textbf{each} \emph{trans in trans\_list(state)}}
      \State $cond\gets get\_condition(trans)$
      \If{$port \in CondDict$}
        \State $CondDict(port)\gets CondDict(port)$.Append($cond$)
      \Else
        \State $CondDict\gets CondDict$.Append($(port,\:cond)$)
      \EndIf
    \EndFor
  \EndFor
  ...
  \For{\textbf{each} \emph{(port, cond\_set) in CondDict}}
    ...
    \State $InputTerm\gets InputTerm$.Append($(port,\:PortTerm)$)
  \EndFor
...
\Function{output\_term}{$synch$}
  ...
  \For{\textbf{each} \emph{state in state\_list(sync)}}\Comment{\emph{Construct a dictionary $DispatchDict$ with output ports as keys and sets of corresponding messages as values}}

    \For{\textbf{each} \emph{trans in trans\_list(state)}}
      \State $send\gets get\_send(trans)$
      \State $(msg,\:port)\gets (get\_msg(send),\:get\_port(send)$
      \If{$port \in DispatchDict$}
        \State $DispatchDict(port)\gets DispatchDict(port)$.Append($msg$)
      \Else
        \State $DispatchDict\gets DispatchDict$.Append($(port,\:msg)$)
      \EndIf
    \EndFor
  \EndFor
  ...
  \For{\textbf{each} \emph{(port,msg\_set) in DispatchDict}}
    ...
    \For{\textbf{each} \emph{msg in msg\_set}}
      ...
      \EndIf
    \EndFor
    \State $OutputTerm\gets OutputTerm$.Append($(port,\:PortTerm)$)
  \EndFor
